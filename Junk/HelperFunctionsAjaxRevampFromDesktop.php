...
function getBusRoutesForRendering($startStop,$endStop,$startOffsetDistance,$endOffsetDistance,$showOnlyIndirectBuses)
{
	//go to the database
  ...
  //  print_r($startBuses);
    ...
    if(sizeof($arrSameBus)>0)
    {
		...
		if($useDepot=="1")
			...
			return displayDirectBusesWithoutFormatting($startStop,$endStop,$sortedDirectBuses,$directDistance);
...
function displayDirectBusesWithoutFormatting($startStop,$endStop,$arrBuses,$directDistance)
{
   // print_r($arrBuses);
	$strRoute='';
	$array_size=sizeof($arrBuses);
	...
	for($i=0;$i<$array_size;$i++)
	{
		...
		$strRoute=$strRoute.'<IsDirectRoute>Y</IsDirectRoute>';
		$strRoute=$strRoute.'<ErrorCode>0</ErrorCode>';		
		$strRoute=$strRoute."<StartStop>".htmlentities($startStop)."</StartStop>";
		$strRoute=$strRoute."<EndStop>".htmlentities($endStop)."</EndStop>";
		$strRoute=$strRoute.'<RouteDetails>'.$firstBus.'</RouteDetails>';
		$strRoute=$strRoute.'<RouteFrequency>'.$firstBusFrequency.'</RouteFrequency>';
		$strRoute=$strRoute.'<DirectDistance>'.$directDistance.'</DirectDistance>';
		$strRoute=$strRoute.'</Route>';
