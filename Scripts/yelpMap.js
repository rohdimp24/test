   ...
   google.maps.event.addDomListener(window, 'load', load_method);
    ...
    function getJunctionIconsForRegularStops(firstJunctionName,secondJunctionName,startStop, endStop)
    {
        if(firstJunctionName==secondJunctionName && firstJunctionName!=startStop && secondJunctionName!=endStop)
            return iconJunc+":"+iconJunc; //errorcode 1
        else if(firstJunctionName!=secondJunctionName && firstJunctionName!=startStop && secondJunctionName==endStop)
            return iconJunc+":"+iconDest; //errorcode 2
        else if(firstJunctionName!=secondJunctionName && firstJunctionName==startStop && secondJunctionName!=endStop)
            return iconSrc+":"+iconJunc; //errorCode 4
        else
            return iconJunc1+":"+iconJunc2; //errorCode 3

    }

    function junctionBusesForToolTipForRegularStops(firstJunctionName,secondJunctionName,startStop, endStop,
                                                    sourceBuses,destBuses,junctionBusesForToolTipString)
    {
        toolTips=junctionBusesForToolTipString.split("^");
        if(firstJunctionName==secondJunctionName && firstJunctionName!=startStop && secondJunctionName!=endStop)
            return toolTips[0]+"^"+toolTips[1]; //errorCode 1
        else if(firstJunctionName!=secondJunctionName && firstJunctionName!=startStop && secondJunctionName==endStop)
            return toolTips[0]+"^"+destBuses;//errorCode 2
        else if(firstJunctionName!=secondJunctionName && firstJunctionName==startStop && secondJunctionName!=endStop)
            return sourceBuses+"^"+toolTips[1];//errorCode 4
        else
            return toolTips[0]+"^"+toolTips[1];//errorCode 3
	...
	function PlotWalkableMap(walkSourceName,walkSourceLat,walkSourceLon,walkDestName,walkDestLat,walkDestLon,walkDistance)
	{
		...
		*/
        ...
        drawDottedLine(sourcePoint,destPoint);
		//create the marker
                                  ...
                                  originalDestinationStop,errorCode)
	{
      ...
      //  var firstJunctionData=localDepotJunctionArray[1].split(":");
        ...
        if(errorCode==10)
        {
            ...
            //draw a line between the firstjunction and the second junction
            drawDottedLine(firstJunctionPoint,secondJunctionPoint);
        ...
        else
        {
            ...
            //drwaw a line between the first junction and the second junction
            drawDottedLine(firstJunctionPoint,secondJunctionPoint);
        }
    ...
    function drawDottedLine(firstJunctionPoint,secJunctionPoint)
    {

        var lineCoordinates = [firstJunctionPoint,secJunctionPoint];
        var lineSymbol = {
            path: 'M 0,-1 0,1',
            strokeOpacity: 1,
            scale: 4
        ...
        var dashedlinePath = new google.maps.Polyline({
        ...
        dashedlinePath.setMap(map);
        polyLines.push(dashedlinePath);
    ...
    
	{
		...
		if(junctionData!=0)
		{
            ...
            junctionIcons=getJunctionIconsForRegularStops(firstJunctionName,secJunctionName,sourceStopName,destStopName).split(":");

            junctionBusesForToolTipArray=junctionBusesForToolTipForRegularStops(firstJunctionName,secJunctionName,
                sourceStopName,destStopName,sourceBuses,destBuses,junctionBusesForToolTip).split("^");
		  ...
		  // add the points
			...
			//if(firstJunctionName!=secJunctionName && sourceStopName!= firstJunctionName && destStopName != secJunctionName)
            if(firstJunctionName!= secJunctionName)
			{
			    //also draw a dashed line between the two junctions suggesting that this is the walking path
				/*var lineCoordinates = [firstJunctionPoint,secJunctionPoint];
				var lineSymbol = {
				  path: 'M 0,-1 0,1',
				  strokeOpacity: 1,
				...
				*/
                ...
                drawDottedLine(firstJunctionPoint,secJunctionPoint);
			}
